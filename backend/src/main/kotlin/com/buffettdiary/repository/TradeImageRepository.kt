...
interface TradeImageRepository : JpaRepository<TradeImage, Long> {
    ...
    @Query("SELECT t.id as id, t.tradeId as tradeId, t.userId as userId, t.fileName as fileName, t.contentType as contentType, t.fileSize as fileSize, t.createdAt as createdAt FROM TradeImage t WHERE t.tradeId IN :tradeIds")
    fun findMetaByTradeIdIn(tradeIds: List<Long>): List<TradeImageMeta>
